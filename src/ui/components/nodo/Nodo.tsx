...
export function Nodo ({ data }: Props) {
  const aspersores = data.aspersores.map(function (aspersorData, i) {
    return (
      <Aspersor key={i} data={aspersorData} />
    )
  })
...
function Aspersor ({ data }: PropsAspersor) {
  ...
  useEffect(() => {
    switch (data.estado) {
      case 'normal':
        setColor('#32CF9C')
        setSpeed('animate-[spin_1s_linear_infinite]')
        break
      case 'warning':
        setColor('#FFC107')
        setSpeed('animate-[spin_2s_linear_infinite]')
        break
      case 'error':
        setColor('#DC3545')
        setSpeed('animate-[spin_4s_linear_infinite]')
        break
      case '':
        setColor('#696767')
