...
export function Trabajo(): JSX.Element {
  ...
  useEffect(() => {
    setHayErrores(logNodos.length > 0)
  }, [logNodos])
      ...
      socket.on('getStateNodo', (nodos) => {
        if (nodos) {
          ...
          nodos
            ...
            .forEach((nodo) =>
              nodo.aspersores
                .filter((f) => f.estado && f.estado?.id != 0)
                .forEach((a) =>
                  logs.push(
                    <li
            ...
            <>
              <Button
                ...
                onClick={() => {
                  ...
                  if (runningJob) {
                    setHabilitar(false)
                    getRPMDeseado('FINA').then((rpmDeseado) => socket.emit('startJob', rpmDeseado))
                  }
                }}
              >
                <svg
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (runningJob) {
                    setHabilitar(false)
                    getRPMDeseado('MEDIA').then((rpmDeseado) => socket.emit('startJob', rpmDeseado))
                  }
                }}
              >
                <svg
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (runningJob) {
                    setHabilitar(false)
                    getRPMDeseado('GRUESA').then((rpmDeseado) =>
                      socket.emit('startJob', rpmDeseado)
                    )
                  }
                }}
        ...
        {modeApp === 'light' && (
          <div
            className={clsx(
              ...
              {
                'animate-pulse': hayErrores,
                'border-error': hayErrores
              }
            )}
          >
            {hayErrores && (
            ...
            <ul className="overflow-hidden h-[140px] w-full pt-10">{logNodos}</ul>
          </div>
        )}
        {modeApp === 'full' && (
          <div
