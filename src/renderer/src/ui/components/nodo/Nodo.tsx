...
import { DetailNodo, DetailNodoProps } from './DetailNodo'
...
export function Nodo({ data: nodoData, posicion, animacion = false }: Props): JSX.Element {
  const { dataLang } = useLang()
  const { getStateModal, addModal, toggleOpenedState } = useModal()

  const { updateNodosConfig } = useNodosStatus()
  ...
  const handleClickNodo = () => {
    console.log('handleClickNodo', nodoData.nombre)
  ...
  const nodoDeshabilitadoChange = async (value: boolean): Promise<void> => {
    if (value !== nodoData.deshabilitado) {
      await window.api.invoke.cambiarHabilitacionNodo(nodoData.id)
      updateNodosConfig()
    }
  }

  const aspersorDeshabilitadoChange = async (idAspersor: number, value: boolean): Promise<void> => {
    if (value !== nodoData.aspersores.find(a => a.id === idAspersor)?.deshabilitado) {
      await window.api.invoke.cambiarHabilitacionAspersor(nodoData.id, idAspersor)
      updateNodosConfig()
    }
  }
  ...
  return (
    <>
      <Modal<DetailNodoProps>
        idModal={`detail-nodo-${nodoData.nombre}`}
        ModalContent={DetailNodo}
        modalContentProps={{
          data: nodoData,
          nodoDeshabilitadoChange: nodoDeshabilitadoChange,
          aspersorDeshabilitadoChange: aspersorDeshabilitadoChange
        }}
        closed={modalClosed}
        crossClose
        outsideClose
      ...
      <section
        className={clsx(
          ...
          {
            'opacity-30': nodoData.deshabilitado
          }
        )}
      ...
      >
        <div className="flex flex-col justify-center items-center">
          ...
          <h2 className={clsx(
              "text-[48px] font-bold",
              {
                'text-red-600': !nodoData.deshabilitado && !nodoData.conectado
              }
            )}
          >
            {nodoData.nombre}
        ...
        <div className="flex flex-row justify-around w-full">
          {
            nodoData.aspersores.map((aspersorData, i) => (
              <Aspersor
                key={i}
                posicion={posicion * 4 + aspersorData.id}
                nodoDeshabilitado={nodoData.deshabilitado}
                nodoConectado={nodoData.conectado}
                data={aspersorData}
                animacion={animacion}
              />
            ))
...
interface PropsAspersor {
  data: IAspersorFull
  nodoDeshabilitado: boolean
  nodoConectado: boolean
  posicion: number
  animacion?: boolean
}

function Aspersor({ data, posicion, animacion = false, nodoDeshabilitado, nodoConectado }: PropsAspersor) {

  return (
    <div className="flex flex-col items-center">
      <div 
        className={clsx({
          'animate-[spin_0s_linear_infinite]': nodoDeshabilitado || !animacion || data.estado === -1 || data.rpm == null || data.rpm <= 0,
          'animate-[spin_20s_linear_infinite]': !nodoDeshabilitado && animacion && data.estado === 0 && data.rpm != null && data.rpm > 0,
          'animate-[spin_40s_linear_infinite]': !nodoDeshabilitado && animacion && (data.estado >= 4 && data.estado <= 9) && data.rpm != null && data.rpm > 0,
        })}
      >
        <svg
          width="41"
          ...
          className={clsx({
            'text-[#696767]': nodoDeshabilitado || data.deshabilitado,
            'text-[#32CF9C]': !nodoDeshabilitado && !data.deshabilitado && nodoConectado && data.estado === 0,
            'text-[#DC3545]': !nodoDeshabilitado && !data.deshabilitado && (!nodoConectado || (data.estado === 1 || data.estado === 2 || data.estado === 3)),
            'text-[#FFC107]': !nodoDeshabilitado && !data.deshabilitado && nodoConectado && (data.estado >= 4 && data.estado <= 9),
          })}
        >
          <path
            fill='currentColor'
